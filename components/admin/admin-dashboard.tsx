...
import type { ProjectArtifacts, ProcessingLog } from "@/types/platform";
...
export function AdminDashboard({
  ...
  initialStats: { totalUsers: number; totalProjects: number; totalMCPs: number };
  initialProjects: any[];
  adminEmail: string;
  userEmail: string;
}) {
  const [activeTab, setActiveTab] = useState<"projects" | "failed" | "tokens" | "logs">("projects");
  ...
  const metrics = useMemo(() => {
    ...
    const successRate = initialProjects.length > 0 
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/5 pb-6">
          <div>
            ...
            <p className="mt-2 text-muted-foreground text-xs font-mono">
              Signed in as <span className="text-violet-300 font-semibold">{userEmail}</span> (Administrator)
            </p>
          </div>
        ...
        <div className="glass-card neon-border rounded-2xl p-6 border border-white/5 bg-black/40 min-h-[400px]">
          {activeTab === "projects" && (
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold text-lg text-white">Recent Conversions</h3>
                <p className="text-muted-foreground text-xs">A comprehensive operational history of all pasted URLs and generated tools.</p>
              </div>
                ...
                <table className="w-full min-w-[800px] text-left text-xs font-mono">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {initialProjects.map((p) => {
                      const tools = (p.artifacts as ProjectArtifacts | null)?.compressedTools ?? [];
                      return (
                        <tr className="hover:bg-white/5 transition-colors group" key={p.id}>
                          <td className="px-5 py-4 font-semibold text-white truncate max-w-[150px]">{p.name}</td>
                          ...
                          <td className="px-5 py-4 capitalize text-cyan-300">{p.sourceType}</td>
          ...
          {activeTab === "failed" && (
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold text-lg text-white">Sandbox Diagnostics</h3>
                <p className="text-muted-foreground text-xs">Examine stack traces, processing logs, and crawling failures for troubleshooting.</p>
              </div>
              <div className="grid gap-4">
                {initialProjects.filter(p => p.status === "error").length === 0 ? (
                  <div className="text-center py-12 border border-dashed border-white/5 rounded-xl">
                    <CheckCircle className="size-10 text-emerald-400 mx-auto opacity-45" />
                    <p className="mt-3 text-muted-foreground text-sm font-mono">Zero failed pipelines. System operating cleanly.</p>
                  </div>
                ) : (
                  initialProjects.filter(p => p.status === "error").map((p) => {
                    const errorLogs = (p.logs as ProcessingLog[])?.filter(l => l.level === "error") ?? [];
                    return (
                      <div className="rounded-xl border border-red-500/15 bg-red-950/10 p-5 font-mono text-xs space-y-3" key={p.id}>
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-semibold text-white">{p.name}</p>
                            <p className="text-muted-foreground text-[10px] mt-0.5 truncate max-w-[500px]">URL: {p.sourceUrl}</p>
                          </div>
                        ...
                        <div className="rounded-lg bg-black/60 border border-white/5 p-3 text-red-300 max-h-40 overflow-y-auto space-y-1">
                          {errorLogs.length > 0 ? (
                            errorLogs.map(l => (
                              <p key={l.id}>[{l.timestamp}] [{l.phase ?? "PIPELINE"}] {l.message}</p>
                            ))
                          ) : (
                            <p>Pipeline crashed. No fatal error logs recorded.</p>
                          )}
                        </div>
                      </div>
                    );
          ...
          {activeTab === "tokens" && (
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold text-lg text-white">AI Token Consumption</h3>
                <p className="text-muted-foreground text-xs">Track detailed LLM tokens and credit budgets spent across the platform pipeline.</p>
              </div>
              <div className="overflow-x-auto rounded-xl border border-white/5">
                <table className="w-full min-w-[700px] text-left text-xs font-mono">
                  <thead className="border-white/5 border-b bg-white/5 text-muted-foreground uppercase text-[10px] tracking-wider">
                    <tr>
                      ...
                      <th className="px-5 py-4 text-right">Inferred Quality Score</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {initialProjects.map((p) => {
                      const usage = p.tokenUsage as { prompt_tokens?: number, completion_tokens?: number, total_tokens?: number } | null;
                      const artifacts = p.artifacts as ProjectArtifacts | null;
                      return (
                        <tr className="hover:bg-white/5 transition-colors" key={p.id}>
                          <td className="px-5 py-4 font-semibold text-white">{p.name}</td>
                          <td className="px-5 py-4 text-cyan-300">{(usage?.prompt_tokens ?? 0).toLocaleString()}</td>
                          <td className="px-5 py-4 text-pink-300">{(usage?.completion_tokens ?? 0).toLocaleString()}</td>
                          <td className="px-5 py-4 text-white">{(usage?.total_tokens ?? 0).toLocaleString()}</td>
                          <td className="px-5 py-4 text-right text-emerald-400 font-semibold">
                            {artifacts?.qualityScore?.mcpScore ? `${artifacts.qualityScore.mcpScore}%` : "N/A"}
                          </td>
                        </tr>
                      );
                    })}
          ...
          {activeTab === "logs" && (
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold text-lg text-white">Crawl & Extraction Logs</h3>
                <p className="text-muted-foreground text-xs">Global live stream of page crawls and AI extraction pipelines.</p>
              </div>
              <div className="rounded-xl border border-white/5 bg-black/60 p-4 max-h-[500px] overflow-y-auto font-mono text-xs text-violet-200 space-y-1.5 no-scrollbar">
                {initialProjects.flatMap(p => (p.logs as ProcessingLog[] ?? []).map(l => ({ ...l, projectName: p.name }))).sort((a, b) => b.timestamp.localeCompare(a.timestamp)).slice(0, 80).map(l => (
                  <div className="flex gap-3 border-b border-white/5 pb-1.5 last:border-0" key={l.id}>
                    <span className="text-muted-foreground">[{l.timestamp.slice(11, 19)}]</span>
                    <span className="text-cyan-400 font-semibold">[{l.projectName}]</span>
                    <span className={`font-semibold ${
                      l.level === "success" ? "text-emerald-400" : l.level === "error" ? "text-red-400" : "text-violet-300"
                    }`}>
                      {l.message}
                    </span>
                  </div>
...
function MetricCard({
  ...
  return (
    <div className={`rounded-2xl border border-white/5 bg-card/30 p-5 backdrop-blur-xl transition-all duration-300 group hover:-translate-y-0.5 ${glowColors[color]}`}>
